    ...
    mOpenSSL(false)
{
    ...
    mFilterFunc = [](const std::shared_ptr<Request>&, const std::shared_ptr<Response>&) { return true; };
}
...
void Server::openSSL(const char *cert, const char *key, const char *ca) {
    if (cert == nullptr || key == nullptr || mIsRunning)
        return;
    mOpenSSL = true;
    mCertFile = cert;
    mKeyFile = key;
...
bool Server::start()
{
    ...
    if (!mIsRunning)
    {
        ...
        if (mOpenSSL) {
            ...
            bind_opts.ssl_cert = mCertFile.c_str();
            bind_opts.ssl_key = mKeyFile.c_str();
            bind_opts.ssl_ca_cert = mCaCertFile.size() > 0 ? mCaCertFile.c_str() : nullptr;
            mConnection = mg_bind_opt(mManager, mBindAddress.c_str(), ev_handler, this, bind_opts);
        } else {
...
bool Server::handleRequest(const std::shared_ptr<Request> &request, const std::shared_ptr<Response> &response)
{
    ...
    if (false == mFilterFunc(request, response))
        return false;
...
void Server::printStats()
{
    ...
    void Server::registFilter(Server::FilterFunc func) {
        mFilterFunc = func;
    }
    ...
    void Server::setMongooseLog(FILE *file, int level) {
        ::cs_log_set_file(file);
        ::cs_log_set_level((cs_log_level)level);
    }
}
